...
module Cognito
  class AccessToken
    ...
    ISSUER_URL = "https://cognito-idp.#{ENV['AWS_REGION']}.amazonaws.com/#{ENV['AWS_COGNITO_USER_POOL_ID']}"
    JWKS_URL = "https://cognito-idp.#{ENV['AWS_REGION']}.amazonaws.com/#{ENV['AWS_COGNITO_USER_POOL_ID']}/.well-known/jwks.json"

    attr_accessor :kid, :alg, :sub, :token_use, :auth_time, :iss, :exp, :iat, :version, :jti, :client_id, :username
    ...
    def initialize(token)
      ...
      %w[sub token_use auth_time iss exp iat version jti client_id username].each do |name|
        send("#{name}=", payload[name])
      end
      if payload.key?('cognito:username')
        @username = payload['cognito:username']
      end
    end

    validates :kid, :alg, :sub, :token_use, :auth_time, :iss, :exp, :iat, :username, presence: true
    ...
    def json_web_keys
      response = Net::HTTP.get(URI.parse(JWKS_URL))
      JSON.parse(response)
    end
  end
end
